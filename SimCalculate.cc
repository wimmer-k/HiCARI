...
int main(int argc, char* argv[]){
  ...
  while(!feof(infile) && !signal_received){
    ...
    else if(header[0]==MINIBALL_ID){
      if(vl>0)
	cout << "miniball timestamp:\t"<< ts << "\tlength: "<< header[1] << "\thex ts: " <<(hex) << ts << "\tlength: "<< header[1] <<(dec)<< endl;
      MBCrystal* crys;
      if(header[1]==sizeof(MB_clust)){
	bsize = fread(&inbuf_miniball[0], sizeof(MB_clust), 1, infile);
	bytes_read += sizeof(struct MB_clust);
	crys = new MBCrystal(inbuf_miniball[0],ts);
      }
      else{
	cout << "Unknown size for miniball data" << endl;
	break;
      }
      if(bsize>0)
	error = evt->DecodeMiniball(crys,ts);
      if(error){
	cout << "An error ("<<error<<") occured in DecodeMiniball() while processing file: " << InputFile << ". Continuing ..." << endl;
	continue;
      }
    }
...
#ifdef USEMINOS
    else if(header[0]==MINOS_PHYSDATA_ID){
      if(vl>0)
	cout << "minos timestamp:\t"<< ts << "\tlength: "<< header[1] << "\thex: " <<(hex) << ts << "\tlength: "<< header[1] <<(dec) << "\tsizeof(MINOS_DATA): " << sizeof(MINOS_DATA) << endl;
      if(header[1]==sizeof(MINOS_DATA)){
	bsize = fread(&inbuf_minos_physicsdata[0], sizeof(MINOS_DATA), 1, infile);
	bytes_read += sizeof(MINOS_DATA);
      }
      else{
	cout << "Unknown size for MINOS_DATA event" << endl;
	break;
      }
      if(bsize>0)
